...
bool IVarExp::unifyWith(const Exp& other, std::unique_ptr<Substitution>& s) const {
    ...
    if (s) {
        auto substituted = s->apply(self());
        if (this != substituted.get()) {
            return substituted->unifyWith(other, s);
        }
        Exp otherSubstituted = other->substitute(*s);
        if (otherSubstituted->isSameVar(*this)) {
            return true;
        }
        if (otherSubstituted->occursIn(*this)) {
            return false;
        }
        s->addSequentially(this->getName(), otherSubstituted);
        return true;
    }

    if (other->isSameVar(*this)) {
        return true;
    }
    ...
    s->addSequentially(this->getName(), other);
    return true;
#else
...
#ifdef WITH_OPTIMIZATION
    return unifyWith(other.self(), s); // koennte man auch wieder optimieren
#else
    if (other.occursIn(*this)) {
